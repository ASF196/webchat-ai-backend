...
| GET    | `/api/admin/pilot-settings/:token`          | `x-admin-secret` header  | Read a bot's Pilot config |
| PATCH  | `/api/admin/pilot-settings/:token`          | `x-admin-secret` header  | Update a bot's Pilot config |
| GET    | `/api/admin/pilot-analytics/:token`         | `x-admin-secret` header  | Requests / success rate / most-requested destinations |
