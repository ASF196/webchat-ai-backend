...
// Delete a bot and everything tied to its token. Every feature that's been
// added since this app started has its own bot_token-referencing table —
// this list needs to stay in sync with db/index.js, or deleting a bot that
// ever used one of those features fails on a foreign-key constraint and
// the bot silently survives (that bug existed here for a while: AI Pilot's
// three tables were added to the schema but never added to this list).
router.delete('/bots/:token', requireAdmin, async (req, res) => {
  try {
    ...
    await db.prepare('DELETE FROM page_assistant_events WHERE bot_token = ?').run(token);
    await db.prepare('DELETE FROM page_assistant_cache WHERE bot_token = ?').run(token);
    await db.prepare('DELETE FROM page_assistant_settings WHERE bot_token = ?').run(token);
