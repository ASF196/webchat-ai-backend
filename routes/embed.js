...
async function send(presetText){
  ...
  try{
    var r=await fetch(c.apiBase+'/api/chat',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({token:c.token,message:text,history:h.slice(-4)})
    });
...
function closeChat(){chatOpen=false;win.classList.add('hidden');win.classList.remove('expanded');orbBtn.style.display='';}
